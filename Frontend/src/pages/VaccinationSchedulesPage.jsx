...
export default function VaccinationSchedulesPage() {
  ...
  const [form, setForm] = useState({ vaccine_name: '', target_species: 'All', age_days: '', booster_interval_days: '', notes: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    setError('')
    try {
      const payload = { ...form, age_days: form.age_days ? parseInt(form.age_days) : null, booster_interval_days: form.booster_interval_days ? parseInt(form.booster_interval_days) : null }
      await vaccinationService.create(payload)
      setForm({ vaccine_name: '', target_species: 'All', age_days: '', booster_interval_days: '', notes: '' })
      setShowForm(false)
      fetchSchedules()
    } catch (err) {
        ...
        <div className="bg-white rounded-xl shadow overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              ...
              {schedules.length === 0 && <tr><td colSpan="5" className="px-4 py-8 text-center text-gray-500">No schedules defined.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
